...
if(typeSNPs == "all") {
  ...
  write.table(GT_AA_anno_m, file = paste0(wrkDir, "/03-data/SNPs_", typeSNPs, ".tbl"), row.names = F, col.names = T, sep = "\t")
} else if(typeSNPs == "onlyNonCoding") {
  SNVs_AA_notFunctional <- GT_AA_anno_m %>% filter(is.na(POS))
  write.table(SNVs_AA_notFunctional, file = paste0(wrkDir, "/03-data/SNPs_", typeSNPs, ".tbl"), row.names = F, col.names = T, sep = "\t")
}
