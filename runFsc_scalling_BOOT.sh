...
if [[ -z "$multiSFS" ]];
then
	modelDir="${genericName}_2DSFS/"
else 
	modelDir="${genericName}_dSFS/"
fi 
...
then
	...
	for sfsObs in ../${genericName}_jointDAFpop*;do echo $sfsObs;
		sufix=`echo $sfsObs | sed 's/.*\(joint.*\)/\1/'`
		prefix=`echo $sfsObs | sed 's/^..\/\(.*\)_joint.*/\1/'`
		cp $sfsObs ${prefix}_${runNb}_${sufix}
	 done
fi
